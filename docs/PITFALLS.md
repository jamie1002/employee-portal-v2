...
3. **規格寫了、code 沒做，測試不會告訴你。** 登入頁的「查看專案說明」按鈕、雲端冷啟動的「喚醒中」橫幅（`ColdStartBanner`），兩次都是 `docs/UI-SPEC.md` 或 `docs/PITFALLS.md` 早就白紙黑字寫著要有這個功能，程式碼卻從頭到尾沒真的實作——因為根本沒有測試在斷言「這個功能存在」，只有測試斷言「已經寫出來的程式碼行為正確」。**批次驗收不能只看測試綠燈，要拿文件逐條對照畫面，問「這條規格今天有沒有人親手點過」。**
