...
### G8. Windows 本機：後端熱重載後整個服務悄悄死掉

**症狀**：`npm run dev:backend` 啟動的後端，改完 `.py` 檔後 uvicorn 偵測到變更，記錄裡出現一行亂碼的「終止批次工作 (Y/N)?」，接著 `Shutting down`——**之後就再也沒有新的伺服器程序起來**。port 3000 仍被一個不回應的 python 程序佔著，請求一律卡到逾時，`/api/health` 也不回。09-13 一天內連續遇到兩次，都是在編輯檔案之後幾秒發生。

**研判原因**（依現象推論，未追到 uvicorn 原始碼）：npm script 在 Windows 上經由 `cmd.exe` 執行，`--reload` 重啟子程序時送出的 Ctrl+C 事件也被外層的批次處理收到，於是跳出「終止批次工作」的詢問；stdin 不是互動終端機，詢問直接被當成「是」而結束了整條程序鏈，只留下孤兒程序佔住 port。

**處置**：改完後端程式要實測 API 前，**先打一次 `/api/health` 確認活著**；不回應就結束佔著 port 3000 的 `python -m uvicorn app.main:app` 程序再重新啟動。這只影響本機開發，雲端（Render）不用 `--reload`。
...
### I15. eval 紅燈先看完整回答；綠燈底下也可能藏著答錯的題目

**症狀（09-13 第一輪完整 eval）**：兩項門檻紅、三題失敗，全部是新加的權限題。打開 `--save-answers` 存下的完整回答，**三題的回答都完全正確**——紅燈全是判定寫錯：

1. **「權限不足」被拿「文件裡沒有」的規則判定。** 權限題標了 `expect_refusal`，而 `is_refusal()` 認的是「文件裡沒有寫到」這類措辭，認不得「這部分你目前沒有權限查看」。兩者語意也根本不同：誘導題是文件真的沒有答案，權限題是系統有資料、只是這個身分看不到——後者答成「文件裡沒有」反而是錯的。
2. **洩漏檢查比對姓名，但姓名本來就在題目裡。** 「張大同這個月出勤狀況如何？」的正確拒絕句一定會複述「張大同」。

**同一份存檔裡還有一題是真的答錯、卻亮綠燈**：員工問「部門主管看得到其他部門同仁的申請紀錄嗎？」，被回「這部分你目前沒有權限查看」外加一句錯誤規定（前一天同題正確）。原因是 I14 修法加的那段「沒有權限」規則被模型套到了「問規定」的題目上。「該答有答」只看 `is_refusal()`，認不得權限句型，所以沒紅。

**更麻煩的是**：第 19 題（答錯）與第 83 題（答對）的回答**開頭一字不差**。只看回答文字無法分辨，必須搭配題目本身的期望——所以權限句型的判定只能放在 eval（依 `expect_permission_denied` 決定出現它是對是錯），不能併進正式環境的 `is_refusal()`。

**修法**：權限題改標 `expect_permission_denied`，獨立一項門檻；「該答有答」把權限句型也算進過度拒答；姓名在題目中時，洩漏改看「數字＋出勤單位」。新判定先用三份存檔（兩百多則回答）**離線重判**確認只命中該命中的題目，才花配額重跑。

**可遷移的教訓**：
- **紅燈先看 `--save-answers` 的完整回答，不要預設是模型的錯。** 09-12、09-13 兩天共五次紅燈是判定寫錯。
- **綠燈也要抽看回答。** 字串比對式的判定只抓得到它想得到的失敗型態，新規則引入的新失敗型態（套錯句型）天生是它的盲點。
- **每次存下完整回答**：改判定邏輯時可以離線重判，不必燒配額。

### I16. 模型拿到一份「看起來能回答」的工具結果就會用——即使那份資料不是在講問題裡的人

**症狀**：員工問「林小美這週有沒有請假？」，AI 回「林小美這週沒有請假紀錄喔」，下一句又說自己只能查本人的資料。後端記錄 `tools=['get_my_requests']`：模型呼叫了**只查提問者本人**的工具，拿到員工自己的申請單，再把它講成林小美的狀況。

**這不是資料外洩**（工具本身查不到別人，執行層的權限邊界沒被破），**但它是一句關於同事的錯誤事實**，比外洩更難被察覺——使用者沒有理由懷疑。工具說明已經寫了「查詢提問者自己送出的申請單」，模型照樣呼叫。原本的四題權限題都沒涵蓋「員工點名問同事」，eval 看不到。

**修法**：`chat_tools.execute()` 多收原始提問 `question`。題目點名了提問者以外的同事、模型卻呼叫 `_SELF_ONLY_TOOLS` 裡的工具時，**不執行**，直接回結構化拒絕——模型拿不到那份會被張冠李戴的結果，自然無從誤用。主管收到的拒絕措辭刻意不含「沒有權限」：他查得到部門同事，只是選錯工具，對他說沒有權限就是 I14 的錯誤訊息。`get_pending_reviews` 不在擋的範圍（它回的本來就是別人的單子）。

**已知代價**：「我跟林小美同一天請假，我的假核准了嗎？」這種問自己、順口提到同事的問題也會被擋。取捨是寧可請使用者到系統頁面查，也不要冒張冠李戴的風險。

**可遷移的教訓**：與 I14 同一條原則——**「題目在問誰」後端查得出來，就不要靠提示詞叮嚀模型別用錯工具**。另外，權限題組要涵蓋「每個角色 × 點名自己範圍外的人 × 每一類工具」，只測「問部門」會漏掉「問個人」。

### I17. 個人資料題能不能查，不能取決於問法像不像某份規章

**症狀（09-13 使用者實測）**：
- 員工問「我8月有哪幾天異常?」列得出日期；改問「我8月有哪幾天異常?**列出日期**」或「我8月有哪幾天**異動**?」，AI 回「關於個人的出勤紀錄，我目前沒有權限直接查詢」。
- 同一個員工問「我7月有哪幾天異常?」只拿到「1 次遲到、6 次早退」，日期列不出來；8 月卻列得出來。
- 主管問「陳小華8月的異常是哪幾天?」回「無法查詢個別員工的詳細出勤紀錄」。

後端記錄：失敗的那幾題全部是 `source_count=0 tools=[]`。

**根因一：用檢索結果決定要不要給工具。** `chat.ask()` 在檢索落空時直接走不帶工具的 `FALLBACK_PROMPT`，而那段提示還寫著「個人出勤紀錄沒辦法回答」。「異常」這個詞碰巧跟出勤規則文件的相似度超過 0.65，多四個字就掉到門檻以下。**個人資料題本來就不該跟政策文件相似**，拿檢索命中與否當閘門，等於做了一次隱性的意圖分流，而且分錯的正好是工具要服務的那一類。這違反了批 B 自己的 Decision 7「一律帶工具」。

**根因二：明細「最多 10 筆」取成了「最近 10 天」。** `records[:10]` 而資料新到舊排序。8 月的異常碰巧落在最近 10 天內，7 月的異常都在月初。聚合是對的，**聚合的內容沒有對準使用者要問的東西**。

**為什麼 eval 沒抓到**：
1. eval 在檢索落空時**直接標記拒答、不呼叫生成**——理由是「落空路徑必定是拒答」，在落空路徑不帶工具時確實成立，但它同時讓 eval 對這條路徑完全失明。
2. 題庫裡的個人資料題全部碰巧命中檢索。
3. 沒有任何一項判定檢查「該呼叫工具卻沒呼叫」或「日期列得完不完整」。

**修法**：
- 檢索落空與否都帶工具，只差系統提示的底（`build_system_prompt(has_context=False)` 以 `FALLBACK_PROMPT` 為底，仍禁止談規定；第二輪另外說明工具查回的資料要照實回答）。
- 出勤工具回傳完整的異常日期清單（不截斷）；團隊工具每人附異常日期；新增通訊錄工具（「我的部門有哪些人」原本被回沒有權限）。
- eval 落空題也實際走生成；新增「工具使用正確」「異常日期完整」（ground truth 由出勤頁同一支 service 即時算出，**刻意不經過工具**，否則工具算錯時 eval 也跟著錯）「通訊錄答案正確」三項判定。題庫直接收錄使用者失敗的原句。

**可遷移的教訓**：
- **閘門條件跟被閘住的東西要是同一個維度。** 「有沒有檢索到政策文件」是政策問答的條件，拿去決定「能不能查個人資料」是維度錯置。
- **eval 為了省配額跳過的路徑，就是 eval 看不到的路徑。** 當初「必定是拒答」的前提，在後續改動後悄悄不成立了，沒人回頭檢查。
- **使用者實測失敗的原句要原封不動收進題庫**，不要改寫成「比較標準」的問法——改寫掉的正是讓它失敗的那幾個字。

### I18. 工具回傳的資料格式錯了，模型不會發現，只會照講

同一輪修正中補測試時另外抓到兩個問題，都是「三層測試全綠、eval 也看不出來」的類型：

1. **打卡時間是 UTC。** `TIMESTAMPTZ` 從 asyncpg 讀出來是 UTC，工具直接 `strftime("%H:%M")`，09:00 上班被寫成 01:00 回給模型。舊版明細很少被講出來，一直沒人發現；新測試斷言「上班時間 == 09:00」才抓到。專案規則早就寫了「今天是哪一天一律走 `get_business_date`」，但**把時間格式化成字串**這一步沒有對應的規則，是同一個坑的另一個入口。
2. **第二輪模型又要求呼叫工具，`response.text` 拋例外。** 「只有一輪工具往返」原本是靠「拿到結果就要文字」維持，但第二輪仍帶著工具清單（歷史裡有 function_call，不宣告工具 API 會拒絕），模型偶爾會再要一次。使用者問「我主管的分機?」看到 503。改以 `tool_config={"function_calling_config": {"mode": "NONE"}}` 禁止（已確認 `google-generativeai` 0.8.6 支援），禁止後仍偶發空回應（3 次 1 次，直接重打 4 次全部正常），重試一次。

**可遷移的教訓**：**測試要斷言工具回傳的具體值**（09:00、2026-07-02），不要只斷言「有這個欄位」「筆數 ≤ 10」。模型對工具回傳內容是完全信任的，格式錯了它不會起疑，只會把錯的東西講得很自然。
