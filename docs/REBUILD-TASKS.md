...
> **實際結果的落差**：以上四條在十個批次都乾淨通過的情況下，事後比對舊版仍發現五項規格早就寫好、但批次執行時從頭到尾沒做出來的功能（登入頁說明按鈕、冷啟動喚醒橫幅、全公司出勤頁、請假整天勾選、請假即時預估時數）——根因與預防方式見 `docs/PITFALLS.md` H 節第 3 條。**這四條驗收標準本身沒有涵蓋 UI 細節的完整度，只驗證了業務邏輯正確，不代表規格 100%被實作。** 下次規劃類似驗收清單時，務必額外加上「規格逐句對照畫面」這一項，不能只靠這四條就宣告完成。
