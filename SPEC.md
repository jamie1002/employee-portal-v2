...
### 4.12 AI 助理（批 A 政策問答 + 批 B 個人資料查詢）

- 完全唯讀。批 A 依四份公司政策文件（`db/policy_docs/*.md`）回答；批 B 另外提供個人出勤／假別／申請進度與通訊錄的查詢工具（見 §4.13）。**助理不提供任何寫入動作**——不能透過對話打卡、送申請或審核。
- **RAG 流程**：語料切段（`##`／`###` 標題為邊界，表格與引言區塊不可切開，特定 Q&A 章節每題一 chunk）→ Gemini embedding（`gemini-embedding-001`，`output_dimensionality=768`，兩側 L2 正規化，`task_type` 刻意不對稱：文件端 `retrieval_document`／查詢端 `retrieval_query`）→ pgvector 餘弦相似度檢索（`RETRIEVAL_TOP_K=5`，`RETRIEVAL_MIN_SCORE=0.65`，等於門檻視為保留）→ 檢索結果為空時系統提示改以受限的 fallback 提示為底（見下，工具清單不變）→ 生成模型（`gemini-3.1-flash-lite`，`temperature` 讀 `GEMINI_TEMPERATURE`）依系統提示的六條規則回答。
...
- **檢索落空的處理**：系統提示改以受限的 fallback 提示為底，只允許寒暄（早安、你好）、對情緒性訊息表達同理並引導聯繫人資或主管、以及說明超出範圍的問題該去哪裡查；該提示**明確禁止產生任何具體規定、時間、天數、金額或計算方式**——這條路徑沒有檢索依據，講出來的都會是編造。**§4.13 的查詢工具在這條路徑照樣提供**：個人資料題本來就不會跟政策文件相似，若以檢索結果決定要不要給工具，使用者能不能查到自己的出勤會取決於問法碰巧像不像某份規章（見 `docs/PITFALLS.md` I17）。模型沒有呼叫工具時 `kind: "fallback"`；有呼叫時為 `"personal"`，工具查回的真實資料（日期、次數、分機）照實回答，不受上述禁令限制。
...
- **核心原則：助理查得到的資料範圍，等於該角色在前端看得到的範圍。** 達成方式不是在提示詞裡叮嚀模型，而是每一支工具都呼叫**與前端相同的 service 函式**，共用同一套範圍限縮（出勤類一律經 `attendance_scope.resolve_visible_user_ids()`）。工具層不自行撰寫 SQL、不自行重算業務規則。
- **工具清單**（宣告與派工見 `backend/app/services/chat_tools.py`）：

| 工具 | 參數 | 底層 service | 誰看得到 |
| :--- | :--- | :--- | :--- |
| `get_today_status` | 無 | `attendance.get_today()` | 全部角色 |
| `get_my_attendance_summary` | `start_date`、`end_date`、`status?` | `attendance.get_my_records()` | 全部角色 |
| `get_my_leave_quota` | 無 | `leave_quota.get_my_leave_quota()` | 全部角色 |
| `get_my_requests` | `kind?`、`status?` | 三種申請的 `get_my_requests()` | 全部角色 |
| `get_pending_reviews` | `kind?` | 三種申請的 `get_pending()` | manager、admin |
| `get_team_attendance_summary` | `start_date`、`end_date`、`employee_name?`、`department_name?`、`status?` | `attendance.get_all()` | manager、admin（`department_name` 僅 admin） |
| `search_directory` | `employee_name?`（可部分比對）、`department_name?`、`only_my_department?` | `user.list_users()` | 全部角色（範圍為全公司，與 `GET /api/users` 相同） |

- **通訊錄的範圍與欄位比照前端「員工資訊」頁**：任何角色都查得到全公司，回傳欄位為姓名、員工編號、部門、角色、分機、email、到職日，不多給。這支工具不做範圍限縮，也不受下方「點名同事」規則影響；提示詞明寫「通訊錄不受出勤資料的權限限制」，避免模型把「別人的資料 → 沒有權限」套到部門成員名單上。
- **題目點名同事時，只查本人的工具不執行**：題目含提問者以外的使用者姓名，而模型呼叫的是 `get_today_status`／`get_my_attendance_summary`／`get_my_leave_quota`／`get_my_requests` 時，工具層不執行並回結構化拒絕——否則模型會拿提問者自己的紀錄回答同事的狀況（見 `docs/PITFALLS.md` I16）。員工收到「沒有權限查看」的說明；主管與管理員收到「這支查詢只看得到你本人的資料」（他們查得到部門同事，只是選錯工具）。
- **兩層防線**：工具宣告依角色組裝（第一層，屬於 UX，減少注定失敗的往返）；工具執行層自行驗證名稱是否在該角色的允許清單內（**第二層，這才是安全邊界**）。模型有可能被誘導喊出不在清單上的工具名稱，僅靠第一層擋不住——與「前端 `RoleGate` 不是安全邊界」是同一條原則。
- **工具參數不得包含 `user_id`／`department_id`**：模型不知道編號代表誰，只會猜；猜中他人又恰好同部門時權限檢查照樣放行，助理便會拿著甲的資料回答關於乙的問題。指定對象一律用姓名或部門名稱，由後端解析，**查無此人與同名多筆各回不同訊息，絕不任選一筆**。解析出的編號仍要經過範圍限縮。
- **統計由工具層計算**，回給模型的是聚合結果，不是原始出勤列。狀態判定沿用 `attendance.matches_status_filter()`——「正常」必須排除當天早退或未打下班卡的日子，讓模型自己數一定會與畫面上的篩選結果不一致。
  - **異常日期完整列出、不截斷**：個人與團隊出勤工具都依遲到／早退／缺勤／未打下班卡（另列請假）回傳日期清單。「我哪幾天異常」的答案就是這份清單；只給部分明細會讓位在區間前段的異常日期查不到。
  - **明細**：查詢區間不超過 10 天時列出每一天的上下班時間；超過時只列有異常的日子（至多 31 筆）。團隊工具只給日期、不給每日時間，避免全公司一個月的明細讓第二輪輸入暴增。
  - **打卡時間一律轉成台北時間**再回給模型——`TIMESTAMPTZ` 讀出來是 UTC，直接格式化會差 8 小時。
- **相對日期以虛擬時鐘的今天為基準**，注入系統提示；取真實時間會讓助理的答案與畫面上的展示時間對不上。
- **但書規則依數字來源分流**：來自工具回傳的數字**不加**「以系統實際顯示為準」（那本來就是系統顯示的內容）；依政策文件推算而得的數字**仍須附上**。「不加但書」這段規則**只在模型實際呼叫工具後的第二輪提示中出現**——寫進第一輪會讓政策問答的推算但書整片消失（見 `docs/PITFALLS.md`）。
- **越權的提問回 200**，由助理以自然語言說明沒有權限並建議詢問主管或人資，不回 403——使用者只是問了一個問題，丟錯誤畫面等於把權限問題偽裝成故障。
- **工具往返上限**：單次提問最多一輪工具呼叫（同一輪可平行呼叫多支），這是結構上的限制而非設定值：第二輪送回工具結果時以 `function_calling_config.mode = NONE` 禁止模型再呼叫工具，第二輪若偶發空回應則重試一次。整趟問答受 `CHAT_TOTAL_TIMEOUT_SECONDS` 限制，逾時回 503 `CHAT_UNAVAILABLE`。
- **記錄只寫工具名稱**：工具參數（含同事姓名）與回傳內容（含出勤明細）一律不進 log。
...
`kind` 有三種：`"policy"`（依政策文件片段回答）、`"personal"`（本次有呼叫查詢工具，見 §4.13；檢索落空與否皆然）、`"fallback"`（檢索落空且未呼叫工具，走受限的寒暄／同理／引導路徑）。

`sources` 只在 `kind` 為 `"policy"` 時有內容——個人資料查詢的答案來自系統本身，沒有文件出處可引用。`refused` 對 `"personal"` 恆為 `false`：政策拒答的字串比對會把「你這個月沒有遲到」這種正常答案誤判成拒答。
