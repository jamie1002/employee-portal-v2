...
- 完全唯讀。批 A 依四份公司政策文件（`db/policy_docs/*.md`）回答；批 B 另外提供個人出勤／假別／申請進度的查詢工具（見 §4.13）。**助理不提供任何寫入動作**——不能透過對話打卡、送申請或審核。
...
- **統計由工具層計算**，回給模型的是聚合結果與至多 10 筆明細，不是原始出勤列。狀態判定沿用 `attendance.matches_status_filter()`——「正常」必須排除當天早退或未打下班卡的日子，讓模型自己數一定會與畫面上的篩選結果不一致。
...
- **工具往返上限**：單次提問最多一輪工具呼叫（`CHAT_MAX_TOOL_ROUNDS`）；整趟問答受 `CHAT_TOTAL_TIMEOUT_SECONDS` 限制，逾時回 503 `CHAT_UNAVAILABLE`。
...
`kind` 有三種：`"policy"`（依政策文件片段回答）、`"personal"`（本次有呼叫個人資料查詢工具，見 §4.13）、`"fallback"`（檢索落空，走受限的寒暄／同理／引導路徑）。
