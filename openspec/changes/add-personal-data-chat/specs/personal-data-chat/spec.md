...
- **GIVEN** 以 `employee` 身分登入
- **WHEN** 提問「林小美這週有沒有請假？」
- **THEN** 只查本人的工具即使被模型呼叫也不執行，助理說明沒有權限查看，MUST NOT 以提問者自己的紀錄回答同事的狀況

#### Scenario: 詢問權限規定本身不視為越權

- **GIVEN** 以 `employee` 身分登入
- **WHEN** 提問「部門主管看得到其他部門同仁的申請紀錄嗎？」
- **THEN** 助理依政策文件回答規定內容，MUST NOT 回答「這部分你目前沒有權限查看」
