...
三層測試全綠、eval 全部門檻全綠、且以三個展示帳號在畫面上各走一遍個人查詢與越權查詢，
才算完成。


---

## 11. 上線前的實測修正（2026-09-13）

細節與「為什麼」見 `docs/PITFALLS.md` I15～I18、`design.md` Decision 11～12。

- [x] 11.1 完整 eval 第一輪：3 題紅全是判定寫錯；存檔另抓到綠燈下的真退化——員工問
      「主管看得到其他部門的申請紀錄嗎？」被誤套權限拒絕句（`fd8d136` 引入）→ 提示詞補對照例子（`03419ce`）
- [x] 11.2 探針：員工點名問同事時模型拿自己的紀錄回答 → 題目點名同事時只查本人的工具不執行（`ebd0567`）
- [x] 11.3 出勤頁人員篩選標籤「使用者」改「員工」（`47c78e1`，含測試 7 處與 UI-SPEC §3.14 兩處）
- [x] 11.4 使用者實測發現四個問題一起修（`02f1e0c`）：檢索落空時不帶工具、明細只給最近
      10 天、團隊工具沒有日期、沒有通訊錄工具；過程中另修打卡時間為 UTC、第二輪再要工具造成 503
- [x] 11.5 eval 判定修正：權限題改 `expect_permission_denied`、洩漏檢查排除題目中的姓名、
      權限說明與閒聊題不計入引用率；題庫改為 94 題（刪 1 加 8）
- [x] 11.6 第三輪完整 eval：唯一紅燈為閒聊題被計入引用率（判定問題），修正後以存檔離線重判全綠；
      推算但書 10/11（91%，門檻 85%）
- [x] 11.7 使用者本機實測通過 → push（`c6846a1`），CI 三個 job 全綠；雲端以三個展示帳號各問一題驗證
- [x] 11.8 文件：SPEC §4.12／§4.13、README、USER_GUIDE、`docs/PROMPT-ENGINEERING.md`
- [ ] 11.9 `openspec archive add-personal-data-chat`（尚未執行：archive 會搬移目錄，程式碼註解中
      引用 `openspec/changes/add-personal-data-chat/` 的路徑要一併更新）
