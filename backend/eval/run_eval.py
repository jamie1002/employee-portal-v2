...
# 這裡再加一層重試，避免整輪題目只因為一次暫時性節流就整份驗收變紅。
_QUOTA_RETRIES = 3
...
# 題目沒有指定 `as_role` 時模擬的身分。用 employee 是刻意的**最保守選擇**：它拿到的
# 工具清單最小，代表大多數使用者的實際情境。權限題與主管／管理員的題目一律在
# questions.yaml 用 `as_role` 各自指定身分。
...
# 於是「主管問部門出勤被誤告知沒有權限」這個 bug 在三層測試與當時六項門檻全綠的情況下
...
# 的權限說明」與「把權限句型誤套到規定問答上」——09-13 實測「主管看得到其他部門的
# 申請紀錄嗎？」與「我們部門這個月誰遲到最多？」兩題的回答開頭一字不差，都是「這部分你目前沒有權限查看」，但前者
        ...
        if with_generation:
            ...
            for attempt in range(_QUOTA_RETRIES):
                try:
                    ...
                    # 直接判定失敗、整輪就地變紅（當時 72 題的一輪實測有 3 題因此失敗，不重試就沒有
...
def _report(
    ...
    if not generated:
        ...
        # 說明誤套到規定問答上時（09-13「主管看得到其他部門的申請紀錄嗎？」），這一項照樣全綠。主管被誤告知沒有權限
